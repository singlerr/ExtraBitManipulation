...
public class ItemBitWrench extends ItemBitToolBase {
  private static final String[] MODE_TITLES =
      new String[] {"Rotation", "Mirroring", "Translation", "Inversion"};
  public static final String[] MODE_TEXT = new String[] {"Rotate", "Mirror", "Translate", "Invert"};

  public ItemBitWrench(Properties properties, String name) {
  ...
  public void cycleModes(ItemStack stack, boolean forward) {
    ...
    nbt.putInt(NBTKeys.WRENCH_MODE,
        BitToolSettingsHelper.cycleData(nbt.getInt(NBTKeys.WRENCH_MODE), forward,
            MODE_TITLES.length));
  }
  ...
  public InteractionResult useOn(UseOnContext useOnContext) {
    if (useOnContext.getLevel().isClientSide) {
      useWrench(useOnContext.getPlayer().getItemInHand(useOnContext.getHand()),
          useOnContext.getPlayer(), useOnContext.getLevel(), useOnContext.getClickedPos(),
      ...
      ExtraBitManipulation.packetNetwork.sendToServer(
          new PacketUseWrench(useOnContext.getClickedPos(), useOnContext.getClickedFace(),
              Configs.oneBitTypeInversionRequirement,
              KeyBindingsExtraBitManipulation.SHIFT.isKeyDown()));
    }
  ...
  public InteractionResult useWrench(ItemStack stack, Player player, Level world, BlockPos pos,
                                     Direction side, boolean oneBitTypeInversionRequirement,
    ...
    if (api.isBlockChiseled(world, pos)) {
      ...
      int s = (invertDirection ? (mode == 1 ? (side.getClockWise((side.getAxis().isHorizontal()
          ? Direction.UP : player.getDirection()).getAxis())) : side.getOpposite()) :
          side).ordinal();
      ...
      Map<BlockState, Integer> inversionBitTypes = new HashMap<BlockState, Integer>();
      ...
      for (int i = 0; i < 16; i++) {
        for (int j = 0; j < 16; j++) {
          for (int k = 0; k < 16; k++) {
            ...
            if (mode == 2) {
              if (!isAir && ((s == 4 && i == 16 - increment) || (s == 0 && j == 16 - increment) ||
                  (s == 2 && k == 16 - increment)
                  || (s == 5 && i == increment - 1) || (s == 1 && j == increment - 1) ||
                  (s == 3 && k == increment - 1))) {
                canTranslate = false;
              }
      ...
      if (canInvert) {
        ...
        for (Entry<BlockState, Integer> entry : inversionBitTypes.entrySet()) {
      ...
      if (!creativeMode && canInvert) {
        ...
        BlockState invertBitState = invertBit.getState();
        bitCountTake = bitCountEmpty - inversionBitTypes.get(invertBitState);
        inversionBitTypes.put(invertBitState,
            Math.max(0, inversionBitTypes.get(invertBitState) - bitCountEmpty));
        if (bitCountTake > 0) {
          invertBitStack = invertBit.getItemStack(1);
          if (invertBitStack.getItem() == null ||
              BitInventoryHelper.countInventoryBits(api, player, invertBitStack) < bitCountTake) {
            canInvert = false;
          }
        }
      }
      ...
      if (!(mode == 2 && !canTranslate) && !(mode == 3 && !canInvert)) {
            ...
            for (int k = 0; k < 16; k++) {
              ...
              switch (mode) {
                ...
                case 1:
                  ...
                  break;
                case 2:
                  int i2 = i + side.getStepX() * increment2;
                  int j2 = j + side.getStepY() * increment2;
                  int k2 = k + side.getStepZ() * increment2;
        ...
        if (!creativeMode && !world.isClientSide && canInvert) {
          if (bitCountTake > 0) {
            BitInventoryHelper.removeOrAddInventoryBits(api, player, invertBitStack, bitCountTake,
                false);
          }
  ...
  public void appendHoverText(ItemStack stack, @org.jetbrains.annotations.Nullable Level level,
    ...
    if (Screen.hasControlDown()) {
      ...
      tooltip.add(Component.literal(
          "Right click blocks to " + text + (mode == 0 ? " CW." : (mode == 1 ? " front-to-back."
              : (mode == 2 ? " front-to-back." : " their bits.")))));
      if (mode != 3) {
        tooltip.add(Component.literal(shiftText + " right click blocks to " + text +
            (mode == 0 ? " CCW." : (mode == 1 ? " left-to-right." : " towards you."))));
      }

      tooltip.add(Component.literal(shiftText + " mouse wheel to cycle modes."));
...
//    TextComponentTranslation textTrans =
//        new TextComponentTranslation(getUnlocalizedNameInefficiently(stack) + ".name",
//            new Object[0]);
//    return textTrans.getUnformattedText() + " - " + MODE_TITLES[getMode(stack)];
//  }

  private int getMode(ItemStack stack) {
    return ItemStackHelper.getNBTOrNew(stack).getInt(NBTKeys.WRENCH_MODE);
  }
