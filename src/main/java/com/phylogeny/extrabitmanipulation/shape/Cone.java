...
public class Cone extends SlopedSymmetricalShape {
  ...
  public boolean isPointInsideShape(BlockPos pos, int i, int j, int k) {
    ...
    return sculptHollowShape ?
        inShape && !(isPointInCone(y, semiDiameterInset2, dist) && !isPointOffLine(y)) : inShape;
  }

  private boolean isPointInCone(float val, float semiDiameter2, double dist) {
    return dist <= Math.abs(
        ((centerY + (inverted ? -semiDiameter2 : semiDiameter2) - val) * semiDiameter) / height);
  }
