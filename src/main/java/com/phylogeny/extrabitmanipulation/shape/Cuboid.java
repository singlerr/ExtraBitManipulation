...
public class Cuboid extends AsymmetricalShape {
  ...
  public boolean isPointInsideShape(BlockPos pos, int i, int j, int k) {
    ...
    return sculptHollowShape ? inShape && !isPointInsideisCuboid(x, y, z, aInset, bInset, cInset) :
        inShape;
  }

  private boolean isPointInsideisCuboid(float x, float y, float z, float a, float b, float c) {
    return x <= centerX + a && x >= centerX - a && y <= centerY + b && y >= centerY - b &&
        z <= centerZ + c && z >= centerZ - c;
  }
