...
public class PyramidIsoscelesTriangular extends AsymmetricalShape {
  private float offsetCenter, center1Inset, center2Inset, insetMin, insetMax, insetMin2, insetMax2,
      height, cInset2;
  ...
  public void init(float centerX, float centerY, float centerZ, float a, float b, float c,
    ...
    direction %= 6;
    super.init(centerX, centerY, centerZ, a, b, c, direction, sculptHollowShape, wallThickness,
        openEnds);
    ...
    Vector3d offset =
        getInnerTriangularPyramidOffset(centerY, centerZ, offsetCenter, a, b, c, wallThickness);
  ...
  private Vector3d getInnerTriangularPyramidOffset(float centerY, float centerZ, float offsetCenter,
                                                   float a, float b, float c, float wallThickness) {
    ...
    return new Vector3d(0, apexY - interY, apexZ - interZ);
  }
  ...
  public boolean isPointInsideShape(BlockPos pos, int i, int j, int k) {
    ...
    if (isTwisted) {
      ...
      return sculptHollowShape ? inShape &&
          !(isPointInPyramid(y, z, x, center1Inset, center2Inset) && !isPointOffLine(y)) : inShape;
    }
    boolean inShape = isPointInPyramid(y, x, z, centerZ, centerY);
    return sculptHollowShape ?
        inShape && !(isPointInPyramid(y, x, z, center1Inset, center2Inset) && !isPointOffLine(y)) :
        inShape;
  }
  ...
  protected boolean isPointInPyramid(float val, float v1, float v2, float center1, float center2) {
    ...
    if (isTwisted) {
      ...
      float center =
          center1 + (inverted ? (offsetCenter * (1 + (s3 / a))) : (-offsetCenter * (1 - (s3 / a))));
      return isPointInTriangle(v1, v2, centerZ, center, s2, isFlipped ? -s1 : s1);
    }
    float s3 = ((dy + (inverted ? -cInset2 : cInset2)) * c) / height;
    float center =
        center1 + (inverted ? (offsetCenter * (1 + (s3 / c))) : (-offsetCenter * (1 - (s3 / c))));
