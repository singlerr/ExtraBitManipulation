...
public class GuiListChiseledArmor<E> extends GuiListExtended {
  ...
  public GuiListChiseledArmor(GuiChiseledArmor guiChiseledArmor, int height,
                              int top, int bottom, int slotHeight, int offsetX, int listWidth,
  ...
  public boolean mouseReleased(int mouseX, int mouseY, int mouseEvent) {
    ...
    for (int i = 0; i < getSize(); ++i) {
      getListEntry(i).mouseReleased(i, mouseX, mouseY, mouseEvent, relativeX,
          relativeY - i * slotHeight);
    }
  ...
  public int getSlotIndexFromScreenCoords(int posX, int posY) {
    ...
    return posX < getScrollBarX() && posX >= left && posX <= left + getListWidth() && y >= 0 &&
        index >= 0 && index < getSize() ? index : -1;
  }
