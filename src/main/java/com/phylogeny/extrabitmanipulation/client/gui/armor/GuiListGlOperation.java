...
public abstract class GuiListGlOperation<L> extends GuiListChiseledArmor<GlOperation> {
  private static final String[] glComponents =
      new String[] {ChatFormatting.BOLD + "x", ChatFormatting.BOLD + "y", ChatFormatting.BOLD + "z",
          "\u03B1"};

  public GuiListGlOperation(GuiChiseledArmor guiChiseledArmor, int height, int top,
                            int bottom, int slotHeight, int offsetX, int listWidth,
  ...
  @Override
  protected void drawListHeader(int insideLeft, int insideTop, Tesselator tessellatorIn) {
    for (int i = 0; i < glComponents.length; i++) {
      mc.fontRenderer.drawString(glComponents[i],
          (int) (insideLeft + 37 + i * (i == 3 ? 42.5 : 45)), top + 1, -1);
    }
  }
