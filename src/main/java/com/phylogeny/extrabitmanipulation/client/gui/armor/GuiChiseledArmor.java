...
public class GuiChiseledArmor extends GuiContainer {
  public static final ResourceLocation TEXTURE_GUI =
      new ResourceLocation(Reference.MOD_ID, "textures/guis/chiseled_armor.png");
  public static final int HELP_TEXT_BACKGROUNG_COLOR = 1694460416;
  private static final ResourceLocation TEXTURE_ROTATION =
      new ResourceLocation(Reference.MOD_ID, "textures/guis/rotation_large.png");
  private static final ResourceLocation TEXTURE_TRANSLATION =
      new ResourceLocation(Reference.MOD_ID, "textures/guis/translation_large.png");
  private static final ResourceLocation TEXTURE_SCALE =
      new ResourceLocation(Reference.MOD_ID, "textures/guis/scale_large.png");
  private static final ResourceLocation TEXTURE_ILLUMINATION_OFF =
      new ResourceLocation(Reference.MOD_ID, "textures/guis/full_illumination_off.png");
  private static final ResourceLocation TEXTURE_ILLUMINATION_ON =
      new ResourceLocation(Reference.MOD_ID, "textures/guis/full_illumination_on.png");
  private static final ResourceLocation TEXTURE_PLAYER_FOLLOW_CURSOR =
      new ResourceLocation(Reference.MOD_ID, "textures/guis/player_follow_cursor.png");
  private static final ResourceLocation TEXTURE_PLAYER_ROTATE =
      new ResourceLocation(Reference.MOD_ID, "textures/guis/player_rotate.png");
  private static final ResourceLocation TEXTURE_SCALE_PIXEL =
      new ResourceLocation(Reference.MOD_ID, "textures/guis/scale_pixel.png");
  private static final ResourceLocation TEXTURE_SCALE_METER =
      new ResourceLocation(Reference.MOD_ID, "textures/guis/scale_meter.png");
  private static final ResourceLocation TEXTURE_ADD =
      new ResourceLocation(Reference.MOD_ID, "textures/guis/add.png");
  private static final ResourceLocation TEXTURE_DELETE =
      new ResourceLocation(Reference.MOD_ID, "textures/guis/delete.png");
  private static final ResourceLocation TEXTURE_MOVE_UP =
      new ResourceLocation(Reference.MOD_ID, "textures/guis/move_up.png");
  private static final ResourceLocation TEXTURE_MOVE_DOWN =
      new ResourceLocation(Reference.MOD_ID, "textures/guis/move_down.png");
  private static final String[] GL_OPERATION_TITLES =
      new String[] {"Rotation", "Translation", "Scale"};
  private static final String[] GL_OPERATION_DATA_TITLES =
      new String[] {"X component", "Y component", "Z component", "Angle"};
  ...
  private final GuiListArmorItem[][] armorItemLists = new GuiListArmorItem[4][3];
  private final List<GuiListGlOperation>[][] armorItemGlLists = new ArrayList[4][3];
  private final GuiListGlOperation[] globalPreGlLists = new GuiListGlOperation[4];
  private final GuiListGlOperation[] globalPostGlLists = new GuiListGlOperation[4];
  private final DataChiseledArmorPiece[] armorPieces = new DataChiseledArmorPiece[4];
  private final GuiButtonTab[][] tabButtons = new GuiButtonTab[4][4];
  private final GuiButtonTextured[][][] concealmentCheckBoxes = new GuiButtonTextured[4][3][2];
  private final GuiButtonTab[] tabButtonsArmorSet = new GuiButtonTab[5];
  private GuiListGlOperation emptyGlList;
  private GuiButtonSelectTextured buttonFullIlluminationOff, buttonFullIlluminationOn,
      buttonPlayerFollowCursor, buttonPlayerRotate, buttonScalePixel,
      buttonScaleMeter, buttonItemAdd, buttonItemDelete, buttonGlAdd, buttonGlDelete,
      buttonGlMoveUp, buttonGlMoveDown,
  ...
  private final AABB[] boxesData = new AABB[4];
  private boolean playerBoxClicked;
  private final int indexArmorSet;
  ...
  public GuiChiseledArmor(EntityPlayer player) {
    ...
    indexArmorSet = setHasArmor(index) ? index :
        ChiseledArmorSlotsHandler.findNextArmorSetIndex(ChiseledArmorSlotsHandler.COUNT_SETS);
    xSize = 352;
    ySize = 230;
  ...
  public void refreshListsAndSelectEntry(int selectedEntry, boolean isArmorItem,
                                         boolean scrollToEnd, int glListRemovalIndex) {
    GuiListChiseledArmor list =
        isArmorItem ? getSelectedGuiListArmorItem() : getSelectedGuiListGlOperation();
    if (glListRemovalIndex >= 0 &&
        glListRemovalIndex < getSelectedGuiListArmorItemGlOperations().size()) {
  ...
  public void initGui() {
    ...
    buttonFullIlluminationOff =
        createButtonToggled(x, 140, TEXTURE_ILLUMINATION_OFF, "Standard illumination",
            prefix + "render with its faces darkened based on rotation, as it does in-world.");
    buttonFullIlluminationOn =
        createButtonToggled(x, 154, TEXTURE_ILLUMINATION_ON, "Full illumination",
            prefix + "render without darkening any of its faces.");
    String suffix1 = " can be interacted with as follows:\n" + getPointMain("1") +
        " Translated by right-clicking and dragging.\n" +
    ...
    buttonPlayerFollowCursor =
        createButtonToggled(x, 186, TEXTURE_PLAYER_FOLLOW_CURSOR, "Look at cursor",
            prefix +
                "bend and twist to look at the cursor. In this mode, the model cannot be rotated, but" +
                suffix1 + suffix2);
    buttonPlayerRotate =
        createButtonToggled(x, 200, TEXTURE_PLAYER_ROTATE, "Left-click & drag to rotate",
            prefix + "only look forward. " +
                "In this mode, the model" + suffix1 + "\n" + getPointMain("3") +
                " Rotated by left-clicking and dragging." + suffix2);
    ...
    text =
        "Moves the selected GL operation @ in the list above. This is important, as changing the order of operations can have a significant effect." +
            "\n\n" + getPointExample() +
            " translating in the x or z axis can result in a translation is any absolute " +
    ...
    buttonGlMoveUp = createButtonListInteraction(144, y, TEXTURE_MOVE_UP, "Move GL operation up",
        text.replace("@", "up"));
    buttonGlMoveDown =
        createButtonListInteraction(158, y, TEXTURE_MOVE_DOWN, "Move GL operation down",
            text.replace("@", "down"));
    ...
    buttonHelp = new GuiButtonHelp(100, buttonList, guiLeft + xSize - 41, guiTop + 5,
        "Show more explanatory hover text", "Exit help mode");
    buttonScale =
        new GuiButtonSelect(100, guiLeft + 188, guiTop + 127, 17, 12, "", "Armor piece scale",
            -5111553, -5111553);
    buttonScale.setTextOffsetX(1);
    buttonScale.setHoverHelpText(
        "This is the scale of the selected armor piece. Just as all newly imported blocks are scaled by this amount " +
    ...
    for (int i = 0; i < armorPieces.length; i++) {
      ...
      if (ItemStackHelper.isChiseledArmorStack(stack)) {
        armorPieces[i] =
            new DataChiseledArmorPiece(ItemStackHelper.getNBTOrNew(stack), ArmorType.values()[i]);
      }

      ArmorType armorType = ArmorType.values()[i];
      ItemChiseledArmor armorItem =
          stack.getItem() instanceof ItemChiseledArmor ? (ItemChiseledArmor) stack.getItem() :
              getArmorItem(armorType);
      ...
      for (int j = 0; j < movingParts.length; j++) {
        GuiButtonTab tabSub =
            new GuiButtonTab(i * 4 + j + 1, guiLeft, guiTop + 147 + j * 25, 24, 25,
                movingParts[j].getName(), true, 19, 230, 0, 512, TEXTURE_GUI,
                movingParts[j].getIconModels(armorItem.getArmorMaterial()));
        ...
        for (int k = 0; k < 2; k++) {
          ...
          GuiButtonTextured checkBox =
              GuiButtonTextured.createCheckBox(1200 + j + concealmentCheckBoxes[0].length * k,
                  guiLeft + 29 + offsetX, guiTop + 153 + j * 25, 12, 12,
                  "Hide player model " + name);
          checkBox.setHoverHelpText("Model Moving Part" + textOverlay + ": " + name +
              "\n\nPrevents the player model's corresponding moving part" +
              textOverlay.toLowerCase() +
              " from rendering when the armor piece is worn in a vanity slot.");
      ...
      for (int j = 0; j < armorItemLists[0].length; j++) {
        ...
        armorItemLists[i][j] =
            new GuiListArmorItem(this, height, guiTop + 24, guiTop + 124, 20, 38, 49, armorPiece,
                j);
        ...
        for (int k = 0; k < armorItems.size(); k++) {
          armorItemGlList.add(
              createGuiListGlOperation(armorPiece, GlOperationListType.ARMOR_ITEM, j, k));
        }
    ...
    for (int i = 0; i < ChiseledArmorSlotsHandler.COUNT_SETS + 1; i++) {
      ...
      GuiButtonTab tab =
          new GuiButtonTab(1100 + i, guiLeft + xSize - 24, guiTop + 23 + i * 25, 24, 25,
      ...
      if (i == 0) {
        ...
        tab.setHoverHelpText(hoverText, " ", "Armor worn in a vanity slot, which will render " +
            ChatFormatting.BLUE + (i < 2 ? "in place of" : "in addition to") +
            ChatFormatting.WHITE + " the main armor.");
      }
  ...
  private boolean setHasArmor(int indexSet) {
    for (ArmorType armorType : ArmorType.values()) {
      if (ItemStackHelper.isChiseledArmorStack(
          ItemStackHelper.getChiseledArmorStack(ClientHelper.getPlayer(), armorType, indexSet))) {
        return true;
      }
    }
    return false;
  ...
  private ItemChiseledArmor getArmorItem(ArmorType armorType) {
    switch (armorType) {
      case HELMET:
        return (ItemChiseledArmor) ItemsExtraBitManipulation.chiseledHelmetDiamond;
      case CHESTPLATE:
        return (ItemChiseledArmor) ItemsExtraBitManipulation.chiseledChestplateDiamond;
      case LEGGINGS:
        return (ItemChiseledArmor) ItemsExtraBitManipulation.chiseledLeggingsDiamond;
      case BOOTS:
        return (ItemChiseledArmor) ItemsExtraBitManipulation.chiseledBootsDiamond;
      default:
        return null;
    }
  }
  ...
  private ItemStack getArmorStack(int index) {
    return ItemStackHelper.getChiseledArmorStack(ClientHelper.getPlayer(), getArmorSlot(index),
        indexArmorSet);
  }

  private GuiListGlOperation createGuiListGlOperation(DataChiseledArmorPiece armorPiece) {
    return createGuiListGlOperation(armorPiece,
        buttonGlItems.selected ? GlOperationListType.ARMOR_ITEM
            : (buttonGlPre.selected ? GlOperationListType.GLOBAL_PRE :
            GlOperationListType.GLOBAL_POST),
  ...
  private GuiListGlOperation createGuiListGlOperation(@Nullable DataChiseledArmorPiece armorPiece,
                                                      GlOperationListType type) {
    return createGuiListGlOperation(armorPiece, type, 0, 0);
  }

  private GuiListGlOperation createGuiListGlOperation(@Nullable DataChiseledArmorPiece armorPiece,
                                                      GlOperationListType type, int partIndex,
                                                      int armorItemIndex) {
    return type == GlOperationListType.ARMOR_ITEM ? new GuiListGlOperationItem(this, height,
        ...
        : new GuiListGlOperationGlobal(this, height, guiTop + 24, guiTop + 124, 16, 91, 220,
        armorPiece, type == GlOperationListType.GLOBAL_PRE);
  }

  private GuiButtonSelectTextured createButtonAddGlOperation(int offsetX, int offsetY,
  ...
  private GuiButtonSelectTextured createButtonListInteraction(int offsetX, int offsetY,
                                                              ResourceLocation texture,
                                                              String hoverText,
                                                              String hoverHelpText) {
    return createButtonToggled(offsetX, offsetY, 12, 12, hoverText, hoverHelpText, -16739073,
        -16739073, texture);
  }

  private GuiButtonSelectTextured createButtonToggled(int offsetX, int offsetY,
                                                      ResourceLocation texture, String hoverText,
                                                      String hoverHelpText) {
    return createButtonToggled(offsetX, offsetY, 12, 12, hoverText,
        hoverText + "\n\n" + hoverHelpText, -16726016, -8882056, texture);
  }

  private GuiButtonSelectTextured createButtonToggled(int offsetX, int offsetY, int width,
                                                      int height,
                                                      String hoverText, String hoverHelpText,
                                                      int colorSelected, int colorDeselected,
                                                      ResourceLocation texture) {
  ...
  private GuiButtonSelect createButtonGlobalGl(int id, int offsetX, int offsetY, String text) {
    return createButtonGl(id, offsetX, offsetY, text,
        "Performed @ each item's GL operations".replace("@", text.toLowerCase()),
        "Before and after a given item's list of GL operations are performed, a global list of operations are performed. For each moving part " +
            ...
            text.replace("Before", "pre").replace("After", "post") +
            "-operations will be displayed in the area below.");
  }

  private GuiButtonSelect createButtonGl(int id, int offsetX, int offsetY, String text,
                                         String hoverText, String hoverHelpText) {
    GuiButtonSelect button = new GuiButtonSelect(id, guiLeft + offsetX, guiTop + offsetY,
        fontRenderer.getStringWidth(text) + 3,
  ...
  private GuiListGlOperation getSelectedGuiListGlOperation() {
    if (!buttonGlItems.selected) {
      return buttonGlPre.selected ? globalPreGlLists[selectedTabIndex] :
          globalPostGlLists[selectedTabIndex];
    }
  ...
  public void handleMouseInput() throws IOException {
    ...
    Pair<Vec3d, Float> pair =
        GuiHelper.scaleObjectWithMouseWheel(this, boxPlayer, playerTranslation, playerScale, 5.0F,
            40.25F);
  ...
  protected void mouseClickMove(int mouseX, int mouseY, int clickedMouseButton,
    ...
    Triple<Vec3d, Vec3d, Float> triple = GuiHelper.dragObject(clickedMouseButton, deltaX, deltaY,
        playerTranslationInitial, playerRotation, playerScale, 5.0F, 2.0F, 3.0F,
        buttonPlayerRotate.selected);
  ...
  protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
    ...
    if (!buttonAddRotation.visible) {
      ...
      if (!buttonGlAdd.isMouseOver() &&
          (!GuiHelper.isCursorInsideBox(boxGlOperation, mouseX, mouseY)
              || buttonAddRotation.isMouseOver() || buttonAddTranslation.isMouseOver() ||
              buttonAddScale.isMouseOver())) {
        hideAddGlButtons();
      }
    }
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    if (GuiHelper.isCursorInsideBox(boxGlOperation, mouseX, mouseY)) {
      for (int i = 0; i < glOperationsList.getSize(); i++) {
        ...
        if (entry.isElementHovered(buttonHelp.selected)) {
          drawHoveringText(
              entry.getElementHoverText(buttonHelp.selected ? glOperationHoverHelpText : null),
              mouseX, mouseY);
          break;
        }
      }
    ...
    if (buttonHelp.selected) {
      if (GuiHelper.isCursorInsideBox(boxTitleItems, mouseX, mouseY)) {
        drawHoveringText(getArmoritemSlotHoverhelpText("The slots of the list below"), mouseX,
            mouseY);
      } else if (GuiHelper.isCursorInsideBox(boxTitleGlOperations, mouseX, mouseY)) {
        drawHoveringText(
            "The rendered items for the moving parts of armor pieces can have any number of GL operations applied " +
      ...
      for (int i = 0; i < boxesData.length; i++) {
        if (GuiHelper.isCursorInsideBox(boxesData[i], mouseX, mouseY)) {
          drawHoveringText(GL_OPERATION_DATA_TITLES[i] + " of the GL operations below", mouseX,
              mouseY);
          break;
        }
      }
  ...
  private String getArmoritemSlotHoverhelpText(String prefix) {
    return prefix + " contain items of the selected moving part of the selected armor piece (" +
        "currently, the " +
        ((ItemChiseledArmor) getArmorStack(selectedTabIndex).getItem()).MOVING_PARTS[
            selectedSubTabIndex - 1].getName() +
        ...
        " Shift-click to clear the copied item.\n" +
        getPointMain("3") +
        " Middle mouse click in creative mode to get item.\n\nKey presses manipulate slots (along with any associated " +
  ...
  protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        ...
        for (int i = 0; i < checkBoxes.length; i++) {
          ...
          for (int j = 0; j < checkBoxs2.length; j++) {
            ...
            if (checkBox != null && checkBox.visible) {
              drawRect(checkBox.x - (j == 1 || i == selectedSubTabIndex - 1 ? 4 : 2),
                  checkBox.y - 1, checkBox.x + checkBox.width + (j == 0 ? -2 : 1),
                  checkBox.y + checkBox.height + 3, HELP_TEXT_BACKGROUNG_COLOR);
            }
          }
        }
      }
    ...
    for (int i = 0; i < armorItemsList.getSize(); i++) {
      ...
      y = (armorItemsList.top - armorItemsList.getAmountScrolled()) +
          i * armorItemsList.slotHeight + armorItemsList.headerPadding + 3 - guiTop;
      zLevel = 100.0F;
  ...
  protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
    ...
    GlStateManager.translate(playerTranslation.x * (1 / playerScale),
        (float) (playerTranslation.y + 3) * (1 / playerScale), 150);
    ...
    GlStateManager.popMatrix();
    if (buttonHelp.selected) {
      drawRect((int) boxTitleItems.minX, (int) boxTitleItems.minY, (int) boxTitleItems.maxX,
          (int) boxTitleItems.maxY, HELP_TEXT_BACKGROUNG_COLOR);
      drawRect((int) boxTitleGlOperations.minX, (int) boxTitleGlOperations.minY,
          (int) boxTitleGlOperations.maxX, (int) boxTitleGlOperations.maxY,
          HELP_TEXT_BACKGROUNG_COLOR);
      for (AABB box : boxesData) {
        drawRect((int) box.minX, (int) box.minY, (int) box.maxX, (int) box.maxY,
            HELP_TEXT_BACKGROUNG_COLOR);
      }
    }
  }
  ...
  private void glScissorBox(AABB box) {
    GuiHelper.glScissor((int) box.minX, (int) box.minY, (int) (box.maxX - box.minX),
        (int) (box.maxY - box.minY));
  }
  ...
  protected void keyTyped(char typedChar, int keyCode) throws IOException {
    ...
    if (buttonAddRotation.visible) {
      ...
      if (!waitingForServerResponse && affectGlOperationsList &&
        ...
        moveGlOperationInList(Keyboard.isKeyDown(Keyboard.KEY_UP));
          ...
          (affectGlOperationsList || GuiHelper.isCursorInsideBox(boxArmorItem, mouseX, mouseY))
          && (Keyboard.isKeyDown(Keyboard.KEY_DELETE) ||
          isCtrlKeyDown() && (cDown || Keyboard.isKeyDown(Keyboard.KEY_V)))) {
        GuiListChiseledArmor list = affectGlOperationsList ? getSelectedGuiListGlOperation() :
            getSelectedGuiListArmorItem();
        if (cDown) {
          if (affectGlOperationsList) {
            ...
            if (entry != null) {
              ...
              GlOperation.saveListToNBT(copiedArmorItemGlOperations, NBTKeys.ARMOR_GL_OPERATIONS,
                  entry.entryObject.getGlOperations());
            }
          }
        } else {
          if (Keyboard.isKeyDown(Keyboard.KEY_V)) {
            ...
            if (affectGlOperationsList) {
              ...
              GuiListEntryChiseledArmor<ArmorItem> entry =
                  getSelectedGuiListArmorItem().getSelectedListEntry();
              int index = list.getSelectListEntryIndex();
              if (entry != null) {
                addOrRemoveArmorItemListData((GuiListArmorItem) list,
                    index != 0 && index == list.getSize() - 1 ? index - 1 : -1, false);
              }
            }
          }
        }
  ...
  private boolean hideAddGlButtons() {
    return buttonAddRotation.visible =
        buttonAddTranslation.visible = buttonAddScale.visible = false;
  }
  ...
  protected void actionPerformed(GuiButton button) throws IOException {
    ...
    if (buttonAddRotation.visible) {
      if (button == buttonAddRotation || button == buttonAddTranslation ||
          button == buttonAddScale) {
        addGlOperationToList(button == buttonAddRotation ? new GlOperation(GlOperationType.ROTATION)
            : (button == buttonAddTranslation ? new GlOperation(GlOperationType.TRANSLATION) :
            new GlOperation(GlOperationType.SCALE, 1, 1, 1)));
      }
    ...
    if (button.id < 16) {
      ...
      if (!buttonTab.selected) {
        ...
        if (indexSubTab != 0) {
          ...
          BitToolSettingsHelper.setArmorMovingPart(mc.player, stack, indexSubTab - 1,
              BitToolSettingsHelper.getArmorMovingPartConfig(
                  ((ItemChiseledArmor) stack.getItem()).armorType),
      ...
      if (button == buttonItemAdd) {
        ...
        int index = list.getSelectListEntryIndex();
        if (entry != null) {
          addOrRemoveArmorItemListData(list,
              index != 0 && index == list.getSize() - 1 ? index - 1 : -1, button);
        }
      }
      ...
      BitToolSettingsHelper.setArmorScale(mc.player, getArmorStack(selectedTabIndex),
          (Integer.parseInt(buttonScale.displayString.substring(2)) / 2 + 1) % 3,
          Configs.armorScale, getArmorSlot(selectedTabIndex), indexArmorSet);
      updateButtons();
      ...
      ExtraBitManipulation.packetNetwork.sendToServer(
          new PacketSetModelPartConcealed(getArmorSlot(selectedTabIndex),
              indexArmorSet, getModelMovingPart(partIndex % len), partIndex / len == 1,
              checkBox.selected));
  ...
  private void removeGlOperationFromListOrEnterAddSelection(boolean add) {
    if ((!buttonGlItems.selected || getSelectedGuiListArmorItem().getSize() > 0)) {
      ...
      if (add) {
        ...
        int index = list.getSelectListEntryIndex();
        setGlOperationListData(list.removeGlOperation(index),
            index != 0 && index == list.getSize() - 1 ? index - 1 : -1, true);
      }
    }
  }
  ...
  private void addGlOperationToList(GlOperation glOperation) {
    ...
    GuiListGlOperation list = getSelectedGuiListGlOperation();
    int index = list.getSelectListEntryIndex();
    setGlOperationListData(list.addGlOperation(list.getSize() > 0 ? ++index : index, glOperation),
  ...
  private void moveGlOperationInList(boolean moveUp) {
    ...
    if (moveUp ? index > 0 : index < list.getSize() - 1) {
      setGlOperationListData(
          list.moveGlOperation(index, (GlOperation) list.getGlOperations().get(index), moveUp),
          index += (moveUp ? -1 : 1), true);
    }
  }
  ...
  private void addOrRemoveArmorItemListData(GuiListArmorItem list, int selectedArmorItem,
                                            GuiButton button) {
    ...
    if (add && scale > 0) {
      ...
      GlOperation.saveListToNBT(nbtGlOperations, NBTKeys.ARMOR_GL_OPERATIONS,
          Collections.singletonList(GlOperation.createScale(scale2, scale2, scale2)));
    }
    setArmorItemListData(list, selectedArmorItem, add ? ListOperation.ADD : ListOperation.REMOVE,
        ItemStack.EMPTY, nbtGlOperations);
  }
  ...
  public void modifyArmorItemListData(int selectedArmorItem, ItemStack stack) {
    NBTTagCompound nbtGlOperations = new NBTTagCompound();
    setArmorItemListData(getSelectedGuiListArmorItem(), selectedArmorItem, ListOperation.MODIFY,
        stack, nbtGlOperations);
  }

  private void setArmorItemListData(GuiListArmorItem list, int selectedArmorItem,
                                    ListOperation listOperation, ItemStack stack,
                                    NBTTagCompound nbtGlOperations) {
    ExtraBitManipulation.packetNetwork.sendToServer(
        new PacketChangeArmorItemList(getArmorSlot(selectedTabIndex), indexArmorSet,
            selectedSubTabIndex - 1,
            list.getSelectListEntryIndex(), selectedArmorItem, listOperation, stack,
            nbtGlOperations, true, null));
    waitingForServerResponse = true;
  }

  public void setGlOperationListData(int selectedGlOperation, boolean refreshLists) {
    setGlOperationListData(getSelectedGuiListGlOperation().getGlOperations(), selectedGlOperation,
        refreshLists);
    waitingForServerResponse = false;
  }

  private void setGlOperationListData(List<GlOperation> glOperations, int selectedGlOperation,
                                      boolean refreshLists) {
    ...
    ExtraBitManipulation.packetNetwork.sendToServer(
        new PacketChangeGlOperationList(nbt, key, getArmorSlot(selectedTabIndex),
            indexArmorSet, selectedSubTabIndex - 1,
            getSelectedGuiListArmorItem().getSelectListEntryIndex(), selectedGlOperation,
            refreshLists, null));
    waitingForServerResponse = true;
  }
  ...
  private void updateButtons() {
    for (int i = 0; i < tabButtons.length; i++) {
      ...
      if (i == selectedTabIndex) {
        int subTab = BitToolSettingsHelper.getArmorMovingPart(nbt,
            (ItemChiseledArmor) getArmorStack(i).getItem()).getPartIndex();
      ...
      for (int j = 0; j < concealmentCheckBoxes[i].length; j++) {
        ...
        for (int k = 0; k < checkBoxes.length; k++) {
          ...
          if (i == selectedTabIndex) {
            ...
            checkBox.selected = modelPartConcealer != null &&
                modelPartConcealer.contains(getModelMovingPart(j), k == 1);
          }
        }
      }
    }
    ...
    String suffix = (buttonGlItems.selected ? "the selected item" :
        "the global " + (buttonGlPre.selected ? "pre" : "post") + "-operations list");
  ...
  private ModelMovingPart getModelMovingPart(int subTabIndex) {
    return getArmorItem(
        ArmorType.values()[selectedTabIndex]).MOVING_PARTS[subTabIndex].getModelMovingPart();
  }

  private enum GlOperationListType {
