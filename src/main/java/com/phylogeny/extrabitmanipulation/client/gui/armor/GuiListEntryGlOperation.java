...
public class GuiListEntryGlOperation<L> extends GuiListEntryChiseledArmor<GlOperation> {
  private static final ResourceLocation TEXTURE_ROTATION =
      new ResourceLocation(Reference.MOD_ID, "textures/guis/rotation.png");
  private static final ResourceLocation TEXTURE_TRANSLATION =
      new ResourceLocation(Reference.MOD_ID, "textures/guis/translation.png");
  private static final ResourceLocation TEXTURE_SCALE =
      new ResourceLocation(Reference.MOD_ID, "textures/guis/scale.png");
  private static int scaleFactor;
  private final List<GuiTextField> dataFields = new ArrayList<GuiTextField>();
  private final GuiButtonExt buttonPlus;
  private final GuiButtonExt buttonMinus;
  ...
  public GuiListEntryGlOperation(GuiListChiseledArmor<GlOperation> listChiseledArmor,
                                 GlOperation entryObject, int index) {
    super(listChiseledArmor, entryObject);
    int len = entryObject.getType() == GlOperationType.ROTATION ? 4 : 3;
    for (int i = 0; i < len; i++) {
      float data = (i == 0 ? entryObject.getX() :
          (i == 1 ? entryObject.getY() : (i == 2 ? entryObject.getZ() : entryObject.getAngle())));
      if (entryObject.getType() == GlOperationType.TRANSLATION && i < 3 &&
          listChiseledArmor.guiChiseledArmor.scalePixel()) {
        data /= Utility.PIXEL_F;
      }

      dataFields.add(createDataField(listChiseledArmor, 20 + i * 45, i == 3 ? 21 : 35,
          getDataFieldString(data)));
    }
    ...
    switch (entryObject.getType()) {
      case ROTATION:
        textureIcon = TEXTURE_ROTATION;
        break;
      case TRANSLATION:
        textureIcon = TEXTURE_TRANSLATION;
        break;
      case SCALE:
        textureIcon = TEXTURE_SCALE;
    }
    this.index = index;
  ...
  private GuiButtonExt createPlusMinusButton(GuiListChiseledArmor<GlOperation> listChiseledArmor,
                                             int offsetX, String text) {
    return new GuiButtonExt(0, listChiseledArmor.left + offsetX, 0, 12, 12, text);
  }

  private GuiTextField createDataField(GuiListChiseledArmor<GlOperation> listChiseledArmor,
                                       int offsetX, int width, String text) {
    GuiTextField field =
        new GuiTextField(6, mc.font, listChiseledArmor.left + offsetX, 0, width, 9);
  ...
  private void changeData(GuiTextField field, int index, String textInitial, boolean changeText) {
    ...
    if (entryObject.getType() == GlOperationType.TRANSLATION && index < 3 &&
        listChiseledArmor.guiChiseledArmor.scalePixel()) {
      data *= Utility.PIXEL_F;
    }
    ...
    switch (index) {
      case 0:
        entryObject.setX(data);
        break;
      case 1:
        entryObject.setY(data);
        break;
      case 2:
        entryObject.setZ(data);
        break;
      case 3:
        entryObject.setAngle(data);
    }
  ...
  public void updateScreen(boolean isSelected) {
    int y = -Mth.clamp(listChiseledArmor.getAmountScrolled(), 0, listChiseledArmor.getMaxScroll())
        + listChiseledArmor.slotHeight * index + listChiseledArmor.headerPadding +
        listChiseledArmor.top + 5;
    for (GuiTextField field : dataFields) {
      if (isSelected) {
  ...
  private float alterIncrementAmount(float inc) {
    if (Minecraft.ON_OSX ? Keyboard.isKeyDown(Keyboard.KEY_LMETA) :
        Keyboard.isKeyDown(Keyboard.KEY_LCONTROL)) {
      inc *= 0.5F;
    }

    if (Minecraft.ON_OSX ? Keyboard.isKeyDown(Keyboard.KEY_RMETA) :
  ...
  public String getElementHoverText(@Nullable String hoverHelpText) {
    ...
    return
        "If one of the fields to the left is focused, pressing these buttons will increase or decrease " +
            ...
            "multiplies the base value by:\n" + ChatFormatting.AQUA + "All fields:" +
  ...
  private String getBaseValueText(GlOperationType operationType, String fieldType, String click,
                                  String shiftClick) {
    return ChatFormatting.AQUA + operationType.getName() + " " + fieldType + ":" +
