...
public class GuiBitToolSettingsMenu extends Screen implements ISlider {
  private final List<ButtonsSetting> buttonsSettingList = Lists.newArrayList();
  private final List<SliderSetting> sliderSettingList = Lists.newArrayList();
  private int buttonCount, lineCount;
  private boolean closing;
  private float visibility;
  private Stopwatch timer = Stopwatch.createStarted();

  protected GuiBitToolSettingsMenu() {
  ...
  public void initGui() {
    ...
    if (ItemStackHelper.isBitWrenchStack(stack)) {
      ...
      lineCount = 3;
      addButtonsSettings(new ButtonsSetting.ModelAreaMode(), "Area Mode",
      ...
      addButtonsSettings(new ButtonsSetting.Direction(), "Direction",
          BitToolSettingsHelper.getDirectionNames());
      ...
      if (!sculptingTool.removeBits()) {
        addButtonsSettings(new ButtonsSetting.OffsetShape(), "Shape Placement", "Offset",
            "Centered");
      }
      ...
      addButtonsSettings(new ButtonsSetting.ArmorMovingPart(), "Moving Part",
          ((ItemChiseledArmor) stack.getItem()).MOVING_PART_TITLES);
    }
  }
  ...
  protected void addSliderSetting(SliderSetting sliderSetting, String title) {
    ...
    GuiSliderSetting slider =
        new GuiSliderSetting(buttonCount++, x + 13, y - 1, " Bits", sliderSetting.getMaxValue(),
            sliderSetting.getValue(), this);
  ...
  protected void createLabel(String title, int x, int y) {
    GuiLabel label =
        new GuiLabel(fontRenderer, buttonCount, x - fontRenderer.getStringWidth(title) - 10, y + 1,
            width, 13, -1);
    label.addLine(title);
    labelList.add(label);
  }
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    drawGradientRect(0, 0, width, height, (int) (visibility * 98) << 24,
        (int) (visibility * 128) << 24);
  ...
  protected class GuiSliderSetting extends GuiSlider {
    ...
    public GuiSliderSetting(int id, int xPos, int yPos, String suf, double maxVal,
    ...
    protected void mouseDragged(Minecraft mc, int mouseX, int mouseY) {
      ...
      GuiUtils.drawContinuousTexturedBox(BUTTON_TEXTURES, x + (int) (sliderValue * (width - 8)), y,
          0, 66, 8, height, 200, 20, 2, 3, 2, 2, zLevel);
    }
