...
public abstract class SliderSetting {
  ...
  private AbstractSliderButton plus, minus;
  ...
  private GuiButton createIncrementButton(GuiSliderSetting slider, int offsetX, String text) {
    return new GuiButtonExt(slider.id * 100,
        slider.x - (offsetX > 0 ? slider.height : -slider.width)
  ...
  public static class SemiDiameter extends SliderSetting {
    ...
    protected void setValue(EntityPlayer player, int value) {
      BitToolSettingsHelper.setSemiDiameter(player, player.getHeldItemMainhand(), value,
  ...
  public static class WallThickness extends SliderSetting {
    ...
    @Override
    protected void setValue(EntityPlayer player, int value) {
      BitToolSettingsHelper.setWallThickness(player, player.getHeldItemMainhand(), value,
