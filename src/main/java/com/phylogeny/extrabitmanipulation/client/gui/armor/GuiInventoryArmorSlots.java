...
public class GuiInventoryArmorSlots
    extends EffectRenderingInventoryScreen<ContainerPlayerArmorSlots> {
  private static final ResourceLocation BACKGROUND =
      new ResourceLocation(Reference.MOD_ID, "textures/guis/inventory_chiseled_armor_slots.png");
  private static final ResourceLocation IMAGE =
      new ResourceLocation(Reference.MOD_ID, "textures/jei/images/chiseled_helmet.png");
  ...
  public void initGui() {
    ...
    super.initGui();
    resetGuiLeft();
    buttonHelp = new GuiButtonHelp(100, buttonList, guiLeft + xSize / 2 + 10, guiTop + 10,
        "Show slot/button hover help text", "Exit help mode");
    buttonHelp.selected = helpMode;
    buttonJEI = new GuiButtonExt(100, guiLeft + xSize / 2 - 7, guiTop - 27, 46, 26, "");
    buttonJEI.visible = JeiReferences.isLoaded && helpMode;
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    if (helpMode) {
      ...
      if (slot != null && slot.slotNumber > 45) {
        drawHoveringText(
            "Only Chiseled Armor with items to render can be put in these slots.\n\nArmor warn here will render " +
                ChatFormatting.BLUE + (slot.slotNumber < 50 ? "in place of" : "in addition to") +
                ChatFormatting.WHITE +
                " any normally worn armor, but will not confer any additional protection.", mouseX,
            mouseY);
  ...
  protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
    ...
    GuiInventory.drawEntityOnScreen(guiLeft + 51, guiTop + 75, 30, guiLeft + 51 - oldMouseX,
        guiTop + 75 - 50 - oldMouseY, mc.player);
    ...
    for (int i = 0; i < ChiseledArmorSlotsHandler.COUNT_SETS; i++) {
      for (int j = 0; j < ChiseledArmorSlotsHandler.COUNT_TYPES; j++) {
        if (cap == null ||
            cap.getStackInSlot(j * ChiseledArmorSlotsHandler.COUNT_TYPES + i).isEmpty()) {
          RenderState.renderStateModelIntoGUI(null, ArmorType.values()[i].getIconModel(),
              ItemStack.EMPTY, 0.4F, true, false, guiLeft + 71 + 18 * j + (j == 0 ? 0 : 21),
              guiTop + 6 + i * 18 + ICON_STACK_OFFSETS[i], 0, 0, 1);
        }
      }
    }
    ...
    if (helpMode) {
      ...
      for (int i = 0; i < ChiseledArmorSlotsHandler.COUNT_SETS; i++) {
        for (int j = 0; j < ChiseledArmorSlotsHandler.COUNT_TYPES; j++) {
          Slot slot =
              inventorySlots.inventorySlots.get(46 + i * ChiseledArmorSlotsHandler.COUNT_TYPES + j);
          x = guiLeft + slot.xPos;
          y = guiTop + slot.yPos;
  ...
  protected void keyTyped(char typedChar, int keyCode) throws IOException {
    if (Configs.armorSlotsGuiExitToMainInventory && (keyCode == Keyboard.KEY_ESCAPE ||
