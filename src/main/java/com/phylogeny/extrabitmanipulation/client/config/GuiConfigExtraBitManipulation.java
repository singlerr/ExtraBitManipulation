...
public class GuiConfigExtraBitManipulation extends GuiConfig {
  ...
  private static String getAbridgedConfigPath() {
    String path = GuiConfig.getAbridgedConfigPath(
        ConfigHandlerExtraBitManipulation.configFileClient.toString());
  ...
  private static List<IConfigElement> getConfigElements() {
    ...
    addToolSettingsDummyElements(ConfigHandlerExtraBitManipulation.configFileClient,
        configElementsClient);
    addToolSettingsDummyElements(ConfigHandlerExtraBitManipulation.configFileServer,
        configElementsServer);

    addChildElementsToDummyElement(ConfigHandlerExtraBitManipulation.configFileServer,
        ConfigHandlerExtraBitManipulation.THROWN_BIT_PROPERTIES,
    ...
    addToolDataDummyElements(ConfigHandlerExtraBitManipulation.configFileClient,
        configElementsToolData,
    ...
    addElementsToDummyElement("Tool Data Settings", hoverText.replace("@@@",
            ConfigHandlerExtraBitManipulation.DATA_CATAGORY_MODEL.toLowerCase() +
    ...
    addChildElementsToDummyElement(ConfigHandlerExtraBitManipulation.configFileCommon,
        ConfigHandlerExtraBitManipulation.THROWN_BIT_PROPERTIES,
        "Configures the disabling of igniting/extinguishing entities/blocks when hit with thrown lava/water bits",
        configElementsCommon, CommonEntry.class);
    addDummyElementsOfProcessedChildElementSetsToDummyElement(
        ConfigHandlerExtraBitManipulation.configFileCommon,
        configElementsCommon, Configs.itemPropertyMap, "Item Properties",
        "Configures the damage characteristics and default data of the Bit Tools",
        CommonEntry.class);
    addDummyElementsOfProcessedChildElementSetsToDummyElement(
        ConfigHandlerExtraBitManipulation.configFileCommon,
        configElementsCommon, new HashMap<Item, ConfigNamed>(),
        "Recipes", "Configures the disabling of diamond nugget Ore Dictionary registration",
        RecipeEntry.class);
    addDummyElementsOfProcessedChildElementSetsToDummyElement(
        ConfigHandlerExtraBitManipulation.configFileClient, configElementsClient,
        Configs.itemShapes,
        ClientEntry.class, ConfigHandlerExtraBitManipulation.RENDER_OVERLAYS,
        "Configures the way the Bit Wrench overlays are rendered",
        "Sculpting Tool Shapes",
        "Configures the Sculpting Tools' bit removal/addition shapes/boxes",
        "Rendering",
        "Configures the rendering of the Bit Wrench's overlays and the Sculpting Tools' bit removal/addition shapes/boxes");

    addElementsToDummyElement("Client", "Contains configs only accessed by the client",
        configElements, configElementsClient, null);
    addElementsToDummyElement("Server", "Contains configs only accessed by the server",
        configElements, configElementsServer, null);
    addElementsToDummyElement("Common",
        "Contains configs accessed by both the server and the client (problems will occur " +
            "if these are not the same on both client and server)", configElements,
        configElementsCommon, null);

    return configElements;
  }

  private static void addToolSettingsDummyElements(Configuration configFile,
    ...
    if (isClient) {
      addChildElementsToDummyElement(configFile, ConfigHandlerExtraBitManipulation.ARMOR_SETTINGS,
          "Configures the z-fighting buffer scale amount for Chiseled Armor pieces",
      ...
      String textReplacementBits =
          "Configures" + textStorage + "the procedures for finding replacement bits ";
      ...
      configElementsModelingTool.addAll(
          getChildElements(ConfigHandlerExtraBitManipulation.configFileClient,
              ConfigHandlerExtraBitManipulation.MODELING_TOOL_SETTINGS));
      addChildElementsToDummyElement(ConfigHandlerExtraBitManipulation.configFileClient,
          ConfigHandlerExtraBitManipulation.UNCHISELABLE_BLOCK_STATES,
          textReplacementBits.replace(textStorage, " ") + textUnchiselable,
          configElementsModelingTool, ClientEntry.class);
      addChildElementsToDummyElement(ConfigHandlerExtraBitManipulation.configFileClient,
          ConfigHandlerExtraBitManipulation.INSUFFICIENT_BITS,
          textReplacementBits.replace(textStorage, " ") + textInsufficient,
          configElementsModelingTool, ClientEntry.class);
      addElementsToDummyElement("Modeling Tool Settings",
          textReplacementBits + textUnchiselable + " or " + textInsufficient,
          configElementsToolSettings, configElementsModelingTool, ClientEntry.class);
    }
    addChildElementsToDummyElement(configFile,
        ConfigHandlerExtraBitManipulation.SCULPTING_WRENCH_SETTINGS,
        ...
        configElementsToolSettings, configClass);
    addElementsToDummyElement("Tool Settings",
        "Configures sculpting dimensions, wrench inversion mode, the way bits are handled when removed " +
            "from the world, and the way bit removal/addition areas are displayed. (applies to all sculpting tools -- see 'Item Properties' " +
            "menu for item-specific settings)" + (isClient ?
            ", as well as the way the Modeling Tool stores block states and finds replacement bits" :
            ""),
        configElements, configElementsToolSettings, configClass);
  }

  private static void addToolDataDummyElements(Configuration configFile,
                                               List<IConfigElement> configElementsToolData,
                                               String dataCatagory, String hoverText) {
    ...
    addChildElementsToDummyElement(configFile, defaults, defaults + " " + dataCatagory,
        "Configures " + dataCatagoryLower + " data default values.", childElements,
        ClientEntry.class);
    addChildElementsToDummyElement(configFile, storageLoc, storageLoc + " " + dataCatagory,
        "Configures whether " + dataCatagoryLower +
            " data is stored/assessed on/from individual tools or on/from the modeling data client config file.",
        childElements, ClientEntry.class);
    addChildElementsToDummyElement(configFile, inChat, inChat + " " + dataCatagory,
        "Configures whether changes to " +
            dataCatagoryLower + " data are displayed in chat.", childElements, ClientEntry.class);
    addElementsToDummyElement(dataCatagory + " Data Settings",
        hoverText.replace("@@@", dataCatagoryLower),
        configElementsToolData, childElements, ClientEntry.class);
  }

  private static void addDummyElementsOfProcessedChildElementSetsToDummyElement(
      Configuration configFile, List<IConfigElement> configElements,
      ConfigShapeRender[] config, Class configClass, String... names) {
    List<IConfigElement> childElements = new ArrayList<IConfigElement>();
    int startLen = names.length;
    for (int i = 0; i < startLen - 4; i += 2) {
      addChildElementsToDummyElement(configFile, names[i], names[i + 1], childElements,
          configClass);
    }
    ...
    for (int i = 0; i < endLen - 2; i++) {
      ...
      if (i % 2 == 0) {
        ...
        processedNames[i] =
            "Configures the color/alpha/line-width of the " + getInsertText(shapeIndex) +
                (shapeIndex < 2 ?
                    (" and the bounding box around the " + getInsertText(shapeIndex + 2)) : "")
    ...
    addDummyElementsOfChildElementSetsToDummyElement(configFile, childElements, configClass,
        processedNames);
    addElementsToDummyElement(names[startLen - 2], names[startLen - 1], configElements,
        childElements, configClass);
  }

  private static String getInsertText(int shapeIndex) {
    boolean removeBits = shapeIndex % 2 == 0;
    return (shapeIndex == 0 ? "Straight" : (shapeIndex == 1 ? "Flat" : "Curved")) + " Sculpting " +
        (removeBits ? "Wire's " : "Spade's ") +
        (shapeIndex > 1 ? "spherical/ellipsoidal" : "cubic/cuboidal") + " bit " +
        (removeBits ? "removal" : "addition") + " area";
  }
  ...
  private static void addDummyElementsOfProcessedChildElementSetsToDummyElement(
    ...
    addDummyElementsOfChildElementSetsToDummyElement(configFile, configElements, configClass,
        processedNames);
  }
  ...
  private static void addDummyElementsOfChildElementSetsToDummyElement(Configuration configFile,
    ...
    if (isRecipe) {
      childElements.addAll(
          getChildElements(configFile, ConfigHandlerExtraBitManipulation.RECIPES_DISABLE));
    }
    ...
    for (int i = 0; i < names.length - 2; i += inc) {
      if (isRecipe) {
        addChildElementsToDummyElement(ConfigHandlerExtraBitManipulation.configFileCommon,
            names[i], "Configures the recipe type and configuration for the " + names[i],
            childElements, configClass);
      } else {
        addChildElementsToDummyElement(configFile, names[i], names[i + 1], childElements,
            configClass);
  ...
  private static void addChildElementsToDummyElement(Configuration configFile, String text,
                                                     ...
                                                     List<IConfigElement> configElements,
                                                     Class configClass) {
    addChildElementsToDummyElement(configFile, text, text.toLowerCase(), toolTip, configElements,
        configClass);
  }

  private static void addChildElementsToDummyElement(Configuration configFile, String text,
                                                     String catagory, String toolTip,
                                                     List<IConfigElement> configElements,
                                                     Class configClass) {
    addElementsToDummyElement(text, toolTip, configElements, getChildElements(configFile, catagory),
        configClass);
  }
  ...
  public static class FileEntry extends CategoryEntry {
    ...
    public FileEntry(GuiConfig owningScreen, GuiConfigEntries owningEntryList,
                     IConfigElement prop) {
  ...
  public static class ClientEntry extends FileEntry {
    ...
    public ClientEntry(GuiConfig owningScreen, GuiConfigEntries owningEntryList,
                       IConfigElement prop) {
      super(owningScreen, owningEntryList, prop);
    }
  ...
  public static class ServerEntry extends FileEntry {
    ...
    public ServerEntry(GuiConfig owningScreen, GuiConfigEntries owningEntryList,
                       IConfigElement prop) {
      super(owningScreen, owningEntryList, prop);
    }
  ...
  public static class CommonEntry extends FileEntry {
    ...
    public CommonEntry(GuiConfig owningScreen, GuiConfigEntries owningEntryList,
                       IConfigElement prop) {
      super(owningScreen, owningEntryList, prop);
    }
  ...
  public static class RecipeEntry extends CommonEntry {
    ...
    public RecipeEntry(GuiConfig owningScreen, GuiConfigEntries owningEntryList,
                       IConfigElement prop) {
      super(owningScreen, owningEntryList, prop);
    }
