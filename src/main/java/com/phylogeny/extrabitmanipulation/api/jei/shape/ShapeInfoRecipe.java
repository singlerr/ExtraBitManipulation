...
public class ShapeInfoRecipe extends InfoRecipeBase {
  public static final String[] GRAPHIC_NAMES =
      new String[] {"sphere", "ellipsoid", "circular_cylinder", "elliptic_cylinder",
          "circular_cone",
          "elliptic_cone", "cube", "cuboid", "square_pyramid", "rectangular_pyramid"};
  private final ResourceLocation graphicImageSymetrical;
  private final ResourceLocation graphicImageAsymetrical;
  private final int imageIndex;
  public static String textSolid, textHollow, textClosed, textOpen;

  public static List<ShapeInfoRecipe> create(IGuiHelper guiHelper,
    ...
    for (int i = 0; i < GRAPHIC_NAMES.length / 2; i++) {
      ...
      recipes.add(new ShapeInfoRecipe(guiHelper, sculptingStacks, isNarrow ? 746 : 1070,
          isNarrow ? 615 : 690, ShapeInfoRecipeCategory.NAME, i, isNarrow));
    }
    return recipes;
  }

  public ShapeInfoRecipe(IGuiHelper guiHelper, List<ItemStack> sculptingStacks, int imageWidth,
                         int imageHeight, String catagoryName, int imageIndex, boolean isNarrow) {
    super(guiHelper, sculptingStacks, imageWidth, imageHeight, GRAPHIC_NAMES[imageIndex * 2 + 1],
        GRAPHIC_NAMES[imageIndex * 2 + 1].toLowerCase().replace(" ", "_"),
        catagoryName, isNarrow ? 19 : 2, 22, isNarrow ? 142 : 158, 123, catagoryName);
    this.imageIndex = imageIndex;
    graphicImageSymetrical = new ResourceLocation(Reference.MOD_ID,
        "textures/jei/graphics/" + GRAPHIC_NAMES[imageIndex * 2] + ".png");
    graphicImageAsymetrical = new ResourceLocation(Reference.MOD_ID,
        "textures/jei/graphics/" + GRAPHIC_NAMES[imageIndex * 2 + 1] + ".png");
  ...
  public void drawInfo(Minecraft minecraft, int recipeWidth, int recipeHeight, int mouseX,
    ...
    minecraft.font.draw(textSolid, xPos - minecraft.font.width(textSolid) / 2, yPos,
        Color.black.getRGB());
    xPos = 109;
    minecraft.font.draw(textHollow, xPos - minecraft.font.width(textHollow) / 2, yPos,
        Color.black.getRGB());
    if (imageIndex % 3 != 0) {
      xPos = 79;
      yPos = (isSloped ? 42 : 36);
      minecraft.font.draw(textClosed, xPos - minecraft.font.width(textClosed) / 2, yPos,
          Color.black.getRGB());
      minecraft.font.draw(textOpen, xPos + 55 - minecraft.font.width(textOpen) / 2, yPos,
          Color.black.getRGB());
    }
  }
